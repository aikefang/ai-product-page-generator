...
const migrationsTableName = "_turing_migrations";
...
function ensureMigrationsTable(db) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS "_turing_migrations" (
...
function getAppliedMigrations(db) {
  const rows = db.prepare(`SELECT "name" FROM "_turing_migrations"`).all();
...
function main() {
  ...
  for (const migration of migrations) {
    ...
    try {
      db.exec(sql);
      db.prepare(`INSERT INTO "_turing_migrations" ("name") VALUES (?)`).run(migration.name);
      db.exec("COMMIT");
